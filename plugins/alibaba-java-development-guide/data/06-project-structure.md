...
**1.【强制】** 定义 GAV 遵从以下规则：
1）GroupI d 格式：`com.{公司/BU}.业务线.[子业务线]`，最多 4 级。
   **说明：** `{公司/BU}` 例如：alibaba / taobao / tmall / kaikeba 等 BU 一级；子业务线可选。
   **正例：** `com.taobao.jstorm` 或 `com.alibaba.dubbo.register`
2）ArtifactI d 格式：产品线名-模块名。语义不重复不遗漏，先到中央仓库去查证一下。
...
**8.【参考】** 服务器内部重定向必须使用 forward；外部重定向地址必须使用 URL Broker 生成，否则因线上采用 HTTPS 协议而导致浏览器提示"不安全"。此外，还会带来 URL 维护不一致的问题。

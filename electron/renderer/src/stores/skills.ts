...
export const useSkillsStore = defineStore('skills', {
  ...
  actions: {
    async loadPlugins() { this.loading = true; this.error = null; try { const r = await window.api.listMarketplacePlugins(); if (r.success && r.data) this.plugins = r.data; else this.error = r.error || 'Failed' } catch(e: any) { this.error = e.message } finally { this.loading = false } },
    async loadSkills() { this.loading = true; this.error = null; try { const r = await window.api.listSkills(); if (r.success && r.data) { this.skills = r.data; this.applyFilters() } else this.error = r.error || 'Failed' } catch(e: any) { this.error = e.message } finally { this.loading = false } },
    ...
    async loadSkillContent(p: string) { this.loading = true; try { const r = await window.api.readSkillContent(p); if (r.success && r.data) { this.skillContent = r.data; return r.data } else this.error = r.error || 'Failed' } catch(e: any) { this.error = e.message } finally { this.loading = false } },

# Agent Message Avatar Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add inline SVG avatar icons to chat messages to visually distinguish AI (robot icon) from user (person icon) messages.

**Architecture:** Add avatar SVG elements inside the existing `.chat-message` flex container in `ChatMessageBubble.vue`. AI avatars appear before the bubble (left side), user avatars appear after the bubble (right side). Pure inline SVG + CSS, no external dependencies.

**Tech Stack:** Vue 3 SFC, inline SVG, scoped CSS
...
- Avatar size: 40x40px circular containers
- AI avatar colors: background `#e0f2fe`, icon `#0284c7`
- User avatar colors: background `#f3f4f6`, icon `#6b7280`
- Gap between avatar and bubble: `8px`
- Follow existing coding style: scoped CSS, BEM-like class naming
...
1. **Spec coverage:** Design doc requires: avatar icons next to messages, inline SVG, 40px circles, robot for AI, person for user, correct colors, 8px gap. All covered in Task 1.
2. **Placeholder scan:** No TBD/TODO in steps. All code shown. All commands specified.
3. **Type consistency:** Uses existing `isAgentType` computed, no new types introduced.

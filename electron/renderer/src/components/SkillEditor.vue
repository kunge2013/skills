<template>
  <div class="skill-editor">
    <div class="editor-header">
      <el-button text @click="store.setView('detail')">&#8592; Back</el-button>
...
async function handleSave() { if (!store.selectedSkill) return; saving.value = true; try { const r = await store.saveSkillContent(store.selectedSkill.sourcePath, editorContent.value, expectedMtime.value); if (r.success) { ElMessage.success('Saved'); originalContent.value = editorContent.value; await loadContent() } else if (r.conflict) { await ElMessageBox.confirm('File modified externally. Overwrite?', 'Conflict', { confirmButtonText: 'Overwrite', cancelButtonText: 'Reload' }).then(async () => { const cur = await store.loadSkillContent(store.selectedSkill!.sourcePath); if (cur) { expectedMtime.value = cur.lastModified; await handleSave() } }).catch(() => loadContent()) } else ElMessage.error(r.error || 'Failed') } finally { saving.value = false } }
function handleDiscard() { editorContent.value = originalContent.value; validationErrors.value = []; ElMessage.info('Discarded') }
async function handleValidate() { validating.value = true; try { const r = await store.validateSkillMd(editorContent.value); validationErrors.value = r.errors; if (r.valid) ElMessage.success('Valid') } finally { validating.value = false } }
